...
class TagBook(TimeStampedModel):
    tag_book_name = models.CharField(
        _("Name"),
        max_length=50,
        blank=False,
        unique=True,
        db_index=True,
        help_text="Enter book tag",
    )
    tag_book_slug = AutoSlugField(
        _("Slug"), populate_from="tag_book_name", unique=True, db_index=True, blank=True
    )
...
class CategoryBook(TimeStampedModel):
    category_book_name = models.CharField(
        _("Name"),
        max_length=50,
        blank=False,
        unique=True,
    ...
    category_book_slug = AutoSlugField(
        _("Slug"),
        populate_from="category_book_name",
        unique=True,
        db_index=True,
        blank=True,
    )
...
class BookTitle(TimeStampedModel):
    ...
    book_publisher = models.ForeignKey(
        "publisher.Publisher", verbose_name=(_("Publisher")), on_delete=models.CASCADE
    )
    book_author = models.ForeignKey(
        "author.Author", verbose_name=(_("Author")), on_delete=models.CASCADE
    )
...
class Book(TimeStampedModel):
    ...
    book_language = models.CharField(
        _("Language"), max_length=50, choices=LanguagesChoices.choices, null=True
    )
    book_summary = models.TextField(
        ...
        null=True,
        blank=True,
        help_text="Summary about the book",
    )
    book_pages = models.PositiveIntegerField(_("Pages Count"), default=0)
    book_category = models.ForeignKey(
        CategoryBook,
        verbose_name=(_("Category Book")),
        on_delete=models.SET_NULL,
        null=True,
        related_name="category_book",
    )
    book_tag = models.ManyToManyField(
        TagBook, verbose_name=(_("Tag Book")), null=True, related_name="tag_book"
    )
    book_type = models.CharField(
        _("Type"), max_length=50, choices=BookChoices.choices, null=True
    )
